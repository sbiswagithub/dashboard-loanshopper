//  Color Pallete
//  #c7edf3 - Background light blue
//  #316ccc - Text Bright Blue
//  #999999 - Text gray
//  #e5e5e5 - background light gray
//  #4C5270 - Dropdown option text
//  #834187 - Purple
//  #21b7d3 - Logo pale blue
//  #00b4f1 - Logo bright blue
//  #00b3f0 - Logo bright blue
//  #1f215e - Logo dark blue
//  #3C6088 - Overlay background ocean blue
//  #fcc438 - Border yellow
//  #e5e5e5 - Button background gray color
export const LOGO_BRIGHT_BLUE = "#00b4f1";
export const LOGO_DARK_BLUE = "#1f215e";
export const BACKGROUND_LIGHT_BLUE = "#c7edf3";
export const BACKGROUND_LIGHT_GRAY = "#e5e5e5";
export const TRANSPARENT = '#00000000';
export const BLACK = '#000000';
export const WHITE = '#ffffff';
export const HIGHLIGHTED_YELLOW = '#F7C04A'
export const TEXT_GRAY = '#999999'
export const BORDER_YELLOW = '#fcc438'
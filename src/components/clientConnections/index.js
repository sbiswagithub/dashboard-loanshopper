...
import { LOGO_BRIGHT_BLUE, LOGO_DARK_BLUE, BACKGROUND_LIGHT_GRAY, TRANSPARENT, WHITE } from '../../constants/colors';
import { fetchBrokerAgent, acceptClientConnection, rejectClientConnection, fetchClientConnections, setClientConnections, 
  addConnectionBrokerDetails, callNumber, openUrl, handleFetchError,  onClickApplyButton } from "../../actions";
   ...
   return (
    <Card style={{elevation: 4, width:"100%", alignSelf:"center", borderWidth:0.1, borderColor:{LOGO_DARK_BLUE}, borderRadius:15, marginBottom:"2%" }}>
					<LinearGradient
							...
							style={[styles.background, { borderWidth: 6, borderRadius: 25,borderColor: LOGO_BRIGHT_BLUE}]}
							start={{ x: 0.5, y: 1 }}
							end={{ x: 0.5, y: 0 }}
						/>
      ...
      <View style={{flexDirection:"column",flexGrow:1, justifyContent:"space-between"}}>
        <View style={{flexDirection:"row"}}>
          <Card.Title title={broker.fullName} subtitle={broker?.brokerAgency?.companyDetails?.companyName} subtitleNumberOfLines={1} 
            titleStyle={styles.textMediumBoldLogoDarkBlue} subtitleStyle={[styles.textSmallLogoDarkBlue, {marginRight:3}]}
            style={{flexGrow:1,}} />
              ...
              onPress={() => {
                trackPromise(
                  props.rejectClientConnection(
                    brokerConnection._id,
                    (response) => {
                      props.fetchClientConnections(props.setClientConnections, 
                        (error) => {
        ...
        <Card.Content>
          ...
          <Button icon="link" mode="contained" style={{paddingLeft:"2%", marginLeft:"2%", borderRadius:"10", alignSelf:"center", backgroundColor:{LOGO_DARK_BLUE}}} 
              ...
              trackPromise(
                props.acceptClientConnection(
                  ...
                  (response) => {
                    ...
                    trackPromise(
                      props.fetchClientConnections(onSuccess, onError)
                    )
                  }, 
                  (error) => {
                      props.handleFetchError(error);
        ...
        <View style={[styles.chipsLayout, {justifyContent:"space-between"}]}>
    ...
    render () {
      ...
      if (this.props?.clientConnections)
            for (const [index, clientConnection] of this.props.clientConnections.entries()) {
              ...
              items.push(<BrokerConnection key={clientConnection._id} clientConnection={clientConnection} brokerAgent={brokerAgent} {...this.props} /> )
            }          
    	return (
...
const mapStateToProps = ({ homeReducer, }) => {
  ...
  return { clientConnections, brokerDetailsList };
};

export default connect(mapStateToProps, { fetchBrokerAgent, acceptClientConnection, rejectClientConnection, fetchClientConnections, setClientConnections, 
  addConnectionBrokerDetails, callNumber, openUrl, handleFetchError, onClickApplyButton })(ClientConnections);
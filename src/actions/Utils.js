...
const s3 = new aws.S3({
  accessKeyId: process.env.REACT_APP_PUBLIC_BORROWER_S3_ACCESS_KEY ,
  secretAccessKey: process.env.REACT_APP_PUBLIC_BORROWER_S3_ACCESS_SECRET,
  region: process.env.REACT_APP_PUBLIC_AWS_REGION_CODE,
});
...
export async function uploadToS3(bodyAsBase64, metadata) {
  ...
  return await s3
    .upload({
      Body: bodyAsBase64,
      Bucket: process.env.REACT_APP_PUBLIC_BORROWER_S3_DOCUMENTS_BUCKET,
      Key: key,

...
import { API_BORROWER_MESSAGES_URI, API_BORROWER_RECOMMENDATIONS_URI } from '../constants/apiUrls';
...
const fetchAgentRecommendations = (props, onSuccess) => {
	const uri = `${API_BORROWER_RECOMMENDATIONS_URI}`;
	trackPromise(
		fetch(uri, {
		    method: "GET",
		    headers: { 'Content-Type': 'application/json', 'Authorization': props.accessCode },
...
const deleteAgentRecommendation = (props, onSuccess) => {
	const uri = `${API_BORROWER_RECOMMENDATIONS_URI}` + '/' + props.recommendation._id;
	trackPromise(
		fetch(uri, {
		    method: "DELETE",
			headers: { 'Content-Type': 'application/json', 'Authorization': props.accessCode },

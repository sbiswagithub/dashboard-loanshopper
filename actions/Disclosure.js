import {
	...
	IMMIGRATION_STATUS_SELECTED, EMPLOYEMENT_EXPERIENCE_ADDED,EMPLOYEMENT_EXPERIENCE_UPDATED, EMPLOYEMENT_EXPERIENCE_REMOVED,
...
export const removeCreditCard = (cardId) => {
  return {
	type: CRED_CARD_REMOVED,
	payload: cardId
  };
};
export const addCreditCard = (cardIssuer, cardLimit) => {
  return {
	type: CRED_CARD_ADDED,
	payload: {id: cardIssuer + new Date().getMilliseconds() , cardIssuer : cardIssuer, cardLimit : cardLimit}
  };
};
export const employmentExperienceRemoved = (set) => {
  return {
	type: EMPLOYEMENT_EXPERIENCE_REMOVED,
	payload: set
  };
};

export const employmentExperienceUpdated = (set) => {
  return {
	type: EMPLOYEMENT_EXPERIENCE_UPDATED,
	payload: set
  };
};

export const employmentExperienceAdded = () => {
  return {
	type: EMPLOYEMENT_EXPERIENCE_ADDED,
  };
};
